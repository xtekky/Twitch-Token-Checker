...
class Main:
    ...
    def title(self):
        while True:
            os.system(
                f"title Twitch Checker ^| Valid ~ {self.valid} ^| Invalid ~ {self.valid} ^| Locked ~ {self.valid}"
            )
            time.sleep(1)
        ...
        while True:
            try:
                ...
                response = requests.post(
                    ...
                    proxies={"https": f"http://{_xproxy}", "http": f"http://{_xproxy}"},
                    timeout=20,
                )
                try:
                    if (
                        response.json()[0]["data"]["followUser"]["error"]["code"]
                        == "FORBIDDEN"
                    ):
                        self.locked += 1
                ...
                try:
                    if (
                        response.json()[0]["data"]["followUser"]["follow"]["user"]["id"]
                        == _id
                    ):
                        self.valid += 1
    ...
    def scrape():
        ...
        for x in ["fortnite", "grand theft auto v", "just chatting", "FIFA 22"]:
            try:
                ...
                response = requests.post(
                    "https://gql.twitch.tv/gql", headers=headers, json=data
                )
                jsonx = response.json()
